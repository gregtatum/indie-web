...
describe('GET /music/stream-audio', () => {
  ...
  it('returns 200 and full content when no Range header is sent', async () => {
  ...
  it('returns 206 and the correct byte slice for a range request', async () => {
    const res = await fetch(
      `${server.baseUrl}/music/stream-audio?path=/audio.mp3`,
  ...
  it('handles a mid-file range correctly', async () => {
    const res = await fetch(
      `${server.baseUrl}/music/stream-audio?path=/audio.mp3`,
  ...
  it('returns 416 for a malformed Range header (bytes=-)', async () => {
    const res = await fetch(
      `${server.baseUrl}/music/stream-audio?path=/audio.mp3`,
      { headers: { Range: 'bytes=-' } },
    );
