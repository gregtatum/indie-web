import { titleText, authorText, chordLineStart, directiveLineStart, blankLine, Comment, SectionName, Chord,
        Dialect_noTitle } from "./syntax.grammar.terms.ts"
...
function readSuffixPart(input: InputStream, off: number) {
  let n = input.peek(off)
  if (n < 0) return off
  if (n == Ch.b || n == Ch.Hash || n == Ch.o || n == Ch.x || n == Ch.Plus || n == Ch.Dash ||
      n >= Ch.Zero && n <= Ch.Nine) return off + 1
  let n1 = input.peek(off + 1), n2 = input.peek(off + 2)
  if (n == Ch.n && n1 == Ch.o) return off + 2 // no
  if (n == Ch.s && n1 == Ch.u && n2 == Ch.s) return off + 3 // sus
  if (n == Ch.a && ((n1 == Ch.d && n2 == Ch.d) || (n1 == Ch.u && n2 == Ch.g))) return off + 3 // add/aug
  if (n == Ch.d && (n1 == Ch.i || n1 == Ch.o) && n2 == Ch.m) return off + 3 // dim/dom
  if (n == Ch.u && n1 == Ch.n && n2 == Ch.i && input.peek(off + 3) == Ch.s) return off + 4 // unis
  if (n == Ch.t && n1 == Ch.r && n2 == Ch.i) {
    for (let i = off + 3; i < off + 8; i++) if (input.peek(i) != "triangle".charCodeAt(i)) return off
    return off + 8
  }
  ...
  if (n == Ch.m || n == Ch.M) {
    if (n1 == Ch.a) {
      ...
      if (n2 == Ch.n) {
        if (input.peek(off + 3) == Ch.o && input.peek(off + 4) == Ch.r) return off + 5
        return off + 3
      }
      return off + 2
    }
...
function readSuffixParts(input: InputStream, off: number) {
  for (;;) {
    let next = readSuffixPart(input, off)
...
function readChordSuffix(input: InputStream) {
  ...
  for (;;) {
    if (input.peek(off) == Ch.ParenL) {
      let end = readSuffixParts(input, off + 1)
      if (input.peek(end) != Ch.ParenR) break
      off = end + 1
    } else {
      let end = readSuffixParts(input, off)
      if (end == off) break
      off = end
    }
  }
...
function readChord(input: InputStream) {
  let parens = input.next == Ch.ParenL
  ...
  if (readSymbolNote(input)) {
    ...
    if (input.next == Ch.Slash) {
      input.advance()
      if (!readNumericNote(input) && ((input.next as any) == Ch.b || (input.next as any) == Ch.Hash)) input.advance()
    }
  } else if (input.next == Ch.Slash) {
    input.advance()
    readSymbolNote(input) || readNumeralNote(input) || readNumericNote(input)
  } else if (input.next == Ch.Pipe || input.next == Ch.Dash || input.next == Ch.x) {
    input.advance()
  } else {
    return false
  }
...
function wordChar(ch: number) {
  return ch >= Ch.Zero && ch <= Ch.Nine || ch >= Ch.A && ch <= Ch.Z || ch >= Ch.a && ch <= Ch.z || ch == Ch.Underscore
}
...
function readSectionHeader(input: InputStream) {
  headers: for (let i = 0; i < sectionHeaders.length; i++) {
    let header = sectionHeaders[i], upper = sectionHeadersUpper[i]
    for (let j = 0; j < header.length; j++) {
      let next = input.peek(j)
      if (next != header.charCodeAt(j) && next != upper.charCodeAt(j)) continue headers
    }
    let after = input.peek(header.length)
    if (endOfLine(after) || after == Ch.Space || after == Ch.Colon || after == Ch.ParenL) {
...
export const line = new ExternalTokenizer((input, stack) => {
  let prev = input.peek(-1), startPos = input.pos
